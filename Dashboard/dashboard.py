...
# Data preparation
cuaca_hari_kerja = hour_df[hour_df["workingday"] == 1].groupby("weathersit")[
    "cnt"
].mean()
cuaca_hari_libur = hour_df[hour_df["holiday"] == 0].groupby("weathersit")[
    "cnt"
].mean()
cuaca_pengaruh_df = pd.DataFrame(
    {"Hari Kerja": cuaca_hari_kerja, "Hari Libur": cuaca_hari_libur}
)
...
cuaca_akhir_pekan_kategori.plot(
    kind="bar", color="orange", alpha=0.7, label="Hari Libur", ax=ax3
)
ax3.set_title(
    "Analisis Pengaruh Cuaca dan Kecepatan Angin terhadap Penggunaan Sepeda (Hari Kerja vs. Hari Libur)"
)
